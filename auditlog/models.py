...
class LogEntryManager(models.Manager):
    ...
    def get_for_objects(self, queryset):
        ...
        if isinstance(primary_keys[0], int):
            print(self.filter(content_type=content_type).filter(Q(object_id__in=primary_keys)).distinct().all())
            return (
...
class LogEntry(models.Model):
    ...
    seen_by = models.ManyToManyField(settings.AUTH_USER_MODEL, related_name="seen_by", blank=True, )
